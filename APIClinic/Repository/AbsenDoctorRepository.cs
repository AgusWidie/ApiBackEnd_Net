    ...
    public class AbsenDoctorRepository : IAbsenDoctor
    {
        ...
        public async Task<IEnumerable<AbsenDoctorResponse>> GetAbsenDoctor(AbsenDoctorSearchRequest param, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var TotalPageSize = Math.Ceiling((decimal)absenDoctorList.Count() / (int)param.PageSize);
                param.TotalPageSize = (long)TotalPageSize;
                var result = absenDoctorList.Skip((int)Page * (int)param.PageSize).Take((int)param.PageSize).ToList();
                return result;
            }
            catch (Exception ex)
            {
