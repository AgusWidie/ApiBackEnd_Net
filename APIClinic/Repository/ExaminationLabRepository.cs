    ...
    public class ExaminationLabRepository : IExaminationLab
    {
        ...
        public async Task<IEnumerable<ExaminationLabResponse>> GetExaminationLab(ExaminationLabSearchRequest param, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var TotalPageSize = Math.Ceiling((decimal)examinationLabList.Count() / (int)param.PageSize);
                param.TotalPageSize = (long)TotalPageSize;
                var result = examinationLabList.Skip((int)Page * (int)param.PageSize).Take((int)param.PageSize).ToList();
                return result;
            }
            catch (Exception ex)
            {
