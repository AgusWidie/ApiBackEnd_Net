            ...
            try
            {
                ...
                if (param.Day == null || param.Day == "")
                {
                    scheduleDoctorList = (from branch in _context.Branch
                                          ...
                                          }).OrderBy(x => x.DoctorName).AsNoTracking().ToList();
                }
                ...
                var TotalPageSize = Math.Ceiling((decimal)scheduleDoctorList.Count() / (int)param.PageSize);
                param.TotalPageSize = (long)TotalPageSize;
                var result = scheduleDoctorList.Skip((int)Page * (int)param.PageSize).Take((int)param.PageSize).ToList();
                return result;
            }
            catch (Exception ex)
            {
